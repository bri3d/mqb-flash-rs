...
static S18_CRYPTO: AesCrypto = AesCrypto::new(
    [
        0x98, 0xD3, 0x12, 0x02, 0xE4, 0x8E, 0x38, 0x54, 0xF2, 0xCA, 0x56, 0x15, 0x45, 0xBA, 0x6F,
        0x2F,
    ],
    [
        0xE7, 0x86, 0x12, 0x78, 0xC5, 0x08, 0x53, 0x27, 0x98, 0xBC, 0xA4, 0xFE, 0x45, 0x1D, 0x20,
        0xD1,
    ],
);

const SA2_SCRIPT: &[u8] = &[
    0x68, 0x02, 0x81, 0x4A, 0x10, 0x68, 0x04, 0x93, 0x08, 0x08, 0x20, 0x09, 0x4A, 0x05, 0x87, 0x22,
    0x12, 0x19, 0x54, 0x82, 0x49, 0x93, 0x07, 0x12, 0x20, 0x11, 0x82, 0x4A, 0x05, 0x87, 0x03, 0x11,
...
fn s18_transfer_size_patch(block_num: u8, address: usize) -> usize {
    assert_eq!(
        block_num, 4,
        "Only Block 4 (ASW3) patching is supported for Simos18"
    );
    if address < 0x9600 {
        return 0x100;
    }
