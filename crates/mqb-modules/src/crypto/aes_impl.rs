...
impl BlockCrypto for AesCrypto {
    fn encrypt(&self, data: &[u8]) -> Vec<u8> {
        ...
        let enc =
            Encryptor::<Aes128>::new_from_slices(&self.key, &self.iv).expect("valid key/iv length");
        let out_len = buf.len();
        let encrypted = enc
    ...
    fn decrypt(&self, data: &[u8]) -> Vec<u8> {
        let mut buf = data.to_vec();
        let dec =
            Decryptor::<Aes128>::new_from_slices(&self.key, &self.iv).expect("valid key/iv length");
        let decrypted = dec
...
mod tests {
    ...
    fn aes_round_trip() {
        let crypto = AesCrypto::new(
            [
                0x98, 0xD3, 0x10, 0x45, 0x23, 0xD6, 0x3A, 0x83, 0x20, 0x16, 0x82, 0x20, 0x10, 0x82,
                0x49, 0x49,
            ],
            [
                0xE7, 0x86, 0x20, 0x49, 0x20, 0x49, 0x44, 0x20, 0x69, 0x6E, 0x67, 0x20, 0x20, 0x6E,
                0x44, 0x20,
            ],
        );
