...
/// The status DIDs *plus* the signed identity checksum, in an order that makes
/// the ECU answer it.
///
...
/// Immobilizer challenge, Imo service 1 (DID `0x2E0`).
///
/// Four bytes from `ImoRnd`, generated once and then latched until consumed or
/// the ignition is cycled.
pub const DID_CHALLENGE: u16 = 0x2E0;
/// Signed identity checksum, Imo service 7 (DID `0x2F9`).
pub const DID_IDENTITY_CKS: u16 = 0x2F9;
...
impl ImmoSnapshot {
    ...
    /// The 4-byte challenge from DID `0x2E0`, if the ECU issued one.
    pub fn challenge(&self) -> Option<[u8; 4]> {
        self.raw(DID_CHALLENGE)?.try_into().ok()
    }

    /// The signed identity checksum from DID `0x2F9`, if the ECU answered it.
    ///
    /// Absent unless the snapshot was read with [`IMMO_DIDS_FULL`], since the
    /// ECU refuses this DID until services 1/3/4/9 have run.
    pub fn identity_checksum(&self) -> Option<&[u8]> {
        self.raw(DID_IDENTITY_CKS)
    }

    /// Every DID the ECU answered, for a raw view.
    pub fn dids(&self) -> impl Iterator<Item = (u16, &[u8])> {
        self.dids.iter().map(|(k, v)| (*k, v.as_slice()))
    }
}
